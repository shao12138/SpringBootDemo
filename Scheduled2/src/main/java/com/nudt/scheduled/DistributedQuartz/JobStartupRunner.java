...
import com.nudt.scheduled.Config.QuartzConfig;
import org.quartz.*;
    ...
    public void run(String... args) throws Exception {
        ...
        try {
            ...
            if (null == trigger) {
                ...
                JobDetail jobDetail = JobBuilder.newJob(clazz).withIdentity("job1", JOB_GROUP_NAME).usingJobData("name","任务1").build();
            ...
            if (null == trigger2) {
                ...
                JobDetail jobDetail2 = JobBuilder.newJob(clazz).withIdentity("job2", JOB_GROUP_NAME).usingJobData("name","任务2").build();
                CronScheduleBuilder scheduleBuilder    = CronScheduleBuilder.cronSchedule("0/2 * * * * ?");
                trigger2 = TriggerBuilder.newTrigger().withIdentity("trigger2", TRIGGER_GROUP_NAME)
                        .withSchedule(scheduleBuilder).build();
                scheduler.scheduleJob(jobDetail2, trigger2);
                System.out.println("Quartz 创建了job2:...:{}" + jobDetail2.getKey());
            } else {
